...
총 6부 22장으로 구성된 이 책은 1부에서 리눅스 프로그래밍 환경에 대한 제반 사항과 GNU make를 사용한 프로세스 생성 및 컨트롤 등을 설명하고 있다. 2부에서는 시스템 프로그래밍에 대한 내용으로 프로세스, 시그널, 시스템 호출, 고급 파일 처리 기법 등에 대해 예제 프로그램을 기반으로 설명한다. 3부에서는 리눅스 API와 관련된 주제로 데이터베이스 API, ncurses로 화면 조작, 고급 ncurses 프로그래밍, 사운드 API 등에 대해 기술하고, 4부에서는 파이프와 FIFO, 메모리 공유, 세마포어와 메시지 큐 등 인터프로세스 통신에 대한 내용을 설명하고 있다. 5부와 6부에서는 리눅스 프로그래밍 유틸리티로 디버깅 툴을 사용하는 방법, 소프트웨어 분산, 소스 코드 변환 추적 등에 대해 \u0026lsquo;Music CD 데이터베이스\u0026rsquo;라는 프로젝트를 통해 실습하고 있다.
`}),e.add({id:8,href:"/docs/projects/gnome-kr-l10n/",title:"Gnome Kr L10n",section:"Projects",content:` 그놈 한글화 프로젝트 # 그놈 한글화 프로젝트는 그놈 프로젝트에서 개발하는 애플리케이션을 한글화하는 프로젝트입니다. 여러분이 그놈을 기반으로한 우분투 리눅스를 설치할 때, 한글을 선택하면 모든 메뉴가 한글로 나타나는데, 이 프로젝트의 결과물입니다. 현재 프로젝트 멤버는 다음 같습니다.
...
https://l10n.gnome.org/teams/ko/ `}),e.add({id:10,href:"/docs/gnome_history/gimp_gtk/",title:"GIMP와 GTK+의 역사",section:"Docs",content:`그놈 프로젝트는 김프라는 그림 편집 및 그리기 도구 개발에서 시작한다. 아래 만화를 통해 김프 프로젝트의 시작과 김프에서 어떻게 Gtk+ 프로젝트가 시작되었는지 확인할 수 있다.
...
참고 # A Brief (and Ancient) History of GIMP, gimp.org 김프, 위키피디아 (한국어, 영문) 인터뷰 기사, 1997년 1월, https://www.xach.com/gg/1997/1/profile/1/ 김프 튜토리얼(링크) `}),e.add({id:11,href:"/docs/gnome_history/gnome_kr_home/",title:"그놈 한국 홈페이지 변천사",section:"Docs",content:` 그놈 한국 홈페이지 변천사 # 2007년 2월 20일 # https://web.archive.org/web/20070220235717/http://gnome.or.kr/
2006년 7월 11일 # 2003년 10월 30일 # 2002년 3월 30일 # https://web.archive.org/web/20020524140742/http://gnome.or.kr/
`}),e.add({id:12,href:"/docs/gnome_history/korea/",title:"그놈 한국(GNOME KOREA) 소개",section:"Docs",content:` 그놈 한국(GNOME KOREA) # 그놈 한국 홈페이지에서 오신 것을 환영합니다!
...
#define PSPV_PROGRAM_NAME _(\u0026ldquo;DomPDF HTML Rendering Previewer\u0026rdquo;) #define PSPV_VERSION _(\u0026ldquo;0.3beta1\u0026rdquo;) #define PSPV_LICENSE _(\u0026ldquo;Licensed Under GNU Public License V2 or more.\u0026rdquo;) #define PSPV_COMMENTS _(\u0026ldquo;Simple DomPDF HTML Renderer helper tool\u0026rdquo;) #define PSPV_COPYRIGHT _(\u0026ldquo;2006 (C) LunA_J\`etch\u0026rdquo;)
...
`}),e.add({id:14,href:"/docs/develop/architecture/",title:"그놈(GNOME) 플랫폼 아키텍쳐 개관",section:"Develop",content:` 그놈(GNOME) 플랫폼 아키텍쳐 개관 # 이 문서는 그놈 플랫폼에서 사용하는 구조/기술들에 대한 내용을 다루게 됩니다. 아래의 내용을 바탕으로, 전체적인 그놈 데스크탑 환경의 배경이 되는 아키텍쳐를 이해하여 사용자/어플리케이션 개발자들이 어떠한 방향으로 접근해나가야 할 것인지에 대한 사항을 다루게 됩니다.
...
`}),e.add({id:15,href:"/docs/develop/begin/",title:"그놈(GNOME)개발 시작하기",section:"Develop",content:` 처음 시작하시는분을 위한 가이드 # 이 페이지에서는, 그놈(GNOME) 데스크탑 환경을 기반으로 하는 소프트웨어의 개발이나, 그놈(GNOME)을 더욱 풍성하게 만들어주는 공헌을 해주실 분을 위한 기초적인 가이드라인을 제시합니다.
